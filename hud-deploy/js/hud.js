...
const HudHUD = (() => {
    ...
    function refreshTimerView() {
        ...
        if (isWorking && isResting) {
            ...
            label.className = 'timer-status status-rest';
        } else if (isWorking) {
            label.innerText = '● LIVE WORKING';
            label.className = 'timer-status status-live';
        } else {
            label.innerText = 'TOTAL WORK TIME';
            label.className = 'timer-status status-total';
        }
    }
    ...
    function updateHeroCard() {
        ...
        if (!placeArea || !emptyArea) return;

        if (lastNearbyPlace) {
            placeArea.classList.remove('empty');
            emptyArea.classList.add('hidden');
            document.getElementById('heroPlaceName').innerText = lastNearbyPlace.name || '';
            document.getElementById('heroMemo').innerText      = lastNearbyPlace.memo || '';
        } else {
            placeArea.classList.add('empty');
